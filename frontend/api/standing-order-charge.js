...
export default async function handler(req, res) {
  ...
  const { mosad_number, KevaId, Currency, Amount, Tashloumim, Groupe, Comments, JoinToKevaId } = req.body;
  ...
  const params = new URLSearchParams({ Action: 'TashlumBodedNew', MosadNumber: inst.mosad_number, ApiPassword: inst.api_password, KevaId, Currency: Currency || '1', Amount });
  if (Tashloumim) params.append('Tashloumim', Tashloumim);
  if (Groupe) params.append('Groupe', Groupe);
  if (Comments) params.append('Comments', Comments);
  if (JoinToKevaId) params.append('JoinToKevaId', JoinToKevaId);
